# CVAT で MMDetection YOLOX-s を使う（サーバレス自動アノテーション）

このドキュメントは、CVAT の **Automatic annotation** 用に **OpenMMLab MMDetection の YOLOX-s（COCO 事前学習）** を Nuclio 関数として動かす手順をまとめたものです。

## 概要

- **実装場所**: `serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio/`
- **Nuclio 上の名前**: `pth-openmmlab-mmdetection-yolox-s`（UI 表示名は **YOLOX-s (MMDetection)**）
- **推論**: MMDetection 3.3.0、`mim download` で取得した `yolox_s_8x8_300e_coco` の設定と重み
...
```bash
./serverless/deploy_gpu.sh serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio
```

成功すると `nuctl get function` に `pth-openmmlab-mmdetection-yolox-s` が **ready** で表示されます。
...
```bash
nuctl delete function pth-openmmlab-mmdetection-yolox-s --platform local --force
DOCKER_BUILDKIT=0 ./serverless/deploy_gpu.sh serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio
```
...
```bash
./serverless/deploy_cpu.sh serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio
```
...
2. モデル一覧から **YOLOX-s (MMDetection)** を選択する。
...
- **イメージのビルド**: `function-gpu.yaml` の `directives` で PyTorch ベースイメージに **OpenMIM / mmcv / mmdet** を入れ、`mim download mmdet --config yolox_s_8x8_300e_coco` で設定とチェックポイントを `/opt/nuclio` に配置します。
...
| 症状 | 確認・対処 |
|------|------------|
| 画面に「Automatic annotation failed」 | `docker logs cvat_worker_annotation --tail 80` で HTTP エラー・URL を確認。`docker logs nuclio-nuclio-pth-openmmlab-mmdetection-yolox-s --tail 80` でスタックトレースを確認。 |
...
| パス | 内容 |
|------|------|
| `serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio/main.py` | ハンドラ（推論・JSON 応答） |
| `serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio/function-gpu.yaml` | GPU 用 Nuclio 定義 |
| `serverless/pytorch/openmmlab/mmdetection/yolox_s/nuclio/function.yaml` | CPU 用 Nuclio 定義 |
