...
def _find_assets(work_dir: Path) -> tuple[str, str]:
    configs = sorted(work_dir.glob("rtmdet_s_8xb32-300e_coco.py"))
    ckpts = sorted(work_dir.glob("rtmdet_s_8xb32-300e_coco*.pth"))
    if not configs or not ckpts:
        raise RuntimeError(
            f"Missing RTMDet config or checkpoint in {work_dir}. "
...
def handler(context, event):
    context.logger.info("Run MMDetection RTMDet-s model")
