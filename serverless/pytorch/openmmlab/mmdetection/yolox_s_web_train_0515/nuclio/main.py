...
def _find_assets(work_dir: Path) -> tuple[str, str]:
    """Resolve training config and checkpoint using last_checkpoint (same as MMEngine)."""
    configs = sorted(work_dir.glob("yolox_s_finetune.py"))
    if not configs:
        raise RuntimeError(
            f"Missing yolox_s_finetune.py under {work_dir}. "
            "Run prepare_assets.sh before nuctl deploy."
        )
    last_file = work_dir / "last_checkpoint"
    if not last_file.is_file():
        raise RuntimeError(
            f"Missing last_checkpoint under {work_dir}. Run prepare_assets.sh before nuctl deploy."
        )
    ...
    if not ckpt.is_file():
        raise RuntimeError(
            f"Checkpoint from last_checkpoint not found: {ckpt} "
            f"(declared as {raw!r}). Run prepare_assets.sh."
        )
    return str(configs[0]), str(ckpt)
...
def handler(context, event):
    context.logger.info("Run MMDetection YOLOX-s (web_train finetune) model")
